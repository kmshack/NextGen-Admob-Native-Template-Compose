...
/**
 * Configuration for one independently managed rewarded interstitial ad preload
 * pool.
 *
 ...
 * sizing to the SDK. The library default is one because a rewarded interstitial
 * is usually consumed at a single reward moment.
 ...
 */
data class RewardedInterstitialAdLoadConfig @JvmOverloads constructor(
...
/**
 * Snapshot of a registered rewarded interstitial ad pool.
 *
 ...
 */
data class RewardedInterstitialAdLoadState(
...
/**
 * Listener for a single key registered in [RewardedInterstitialAdLoadManager].
 *
 ...
 * into [RewardedInterstitialAdLoadManager].
 */
interface RewardedInterstitialAdLoadListener {
    fun onStateChanged(key: String, state: RewardedInterstitialAdLoadState) = Unit
...
/**
 * Java-friendly no-op adapter for [RewardedInterstitialAdLoadListener].
 *
 * Kotlin callers can implement [RewardedInterstitialAdLoadListener] directly.
 * Java callers can extend this class and override only the callbacks they need.
 */
abstract class RewardedInterstitialAdLoadListenerAdapter : RewardedInterstitialAdLoadListener {
    override fun onStateChanged(key: String, state: RewardedInterstitialAdLoadState) = Unit
...
/**
 * Application-scoped manager for Next-Gen SDK rewarded interstitial ad preload
 * pools.
 *
 ...
 * the ad after it leaves the screen. Per-show options such as
 * `setServerSideVerificationOptions` or `setImmersiveMode` require [pollAd],
 * because [showAdIfAvailable] never exposes the ad to the caller.
 */
class RewardedInterstitialAdLoadManager private constructor(
    private val gateway: RewardedInterstitialAdPreloaderGateway,
    ...
    constructor() : this(
        gateway = SdkRewardedInterstitialAdPreloaderGateway,
     ...
     */
    fun register(key: String, config: RewardedInterstitialAdLoadConfig) {
        requireValidKey(key)
    ...
    /**
     * Convenience overload that creates [RewardedInterstitialAdLoadConfig] from
     * an ad unit ID.
     */
    @JvmOverloads
    fun register(
        key: String,
        adUnitId: String,
        bufferSize: Int? = RewardedInterstitialAdLoadConfig.DEFAULT_BUFFER_SIZE,
        maxAdAgeMs: Long = RewardedInterstitialAdLoadConfig.DEFAULT_MAX_AD_AGE_MS,
    ) {
        register(
            key = key,
            config = RewardedInterstitialAdLoadConfig(
     ...
     */
    fun registerIfAbsent(key: String, config: RewardedInterstitialAdLoadConfig): Boolean {
        requireValidKey(key)
     ...
     */
    fun pollAd(key: String): RewardedInterstitialAd? {
        requireValidKey(key)
        if (!isSdkInitialized()) return null

        var adForCaller: RewardedInterstitialAd? = null
    ...
    suspend fun awaitAd(
        ...
        return withTimeoutOrNull(timeoutMs) {
            suspendCancellableCoroutine { continuation ->
                lateinit var listener: RewardedInterstitialAdLoadListener
                val completed = AtomicBoolean(false)

                fun finish(ad: RewardedInterstitialAd?) {
                ...
                listener = object : RewardedInterstitialAdLoadListener {
                    override fun onStateChanged(
                        key: String,
                        state: RewardedInterstitialAdLoadState,
                    ) {
                        if (state.isStarted) {
     ...
     * dismissed or fails to show. [onUserEarnedReward] is delivered by the SDK
     * while the ad is on screen and may never fire when the user skips out
     * early, so grant the reward there rather than in [onShowAdComplete].
     * [onShowAdComplete] is always invoked on the main thread exactly once —
     * immediately when no ad is available, the activity is finishing, or
     * another ad from this manager is showing.
     *
    ...
    fun showAdIfAvailable(
        ...
        if (Looper.myLooper() != Looper.getMainLooper()) {
            postToMain {
                showAdIfAvailable(activity, key, onUserEarnedReward, onShowAdComplete)
            }
            return
        }
     ...
     */
    fun getState(key: String): RewardedInterstitialAdLoadState? {
        requireValidKey(key)
        return synchronized(lock) {
    ...
    fun getConfig(key: String): RewardedInterstitialAdLoadConfig? {
        requireValidKey(key)
    ...
    fun addListener(
        key: String,
        listener: RewardedInterstitialAdLoadListener,
    ...
    fun removeListener(key: String, listener: RewardedInterstitialAdLoadListener): Boolean {
        requireValidKey(key)
        return synchronized(lock) {
     ...
     * `RewardedInterstitialAdPreloader.destroyAll()`, which could destroy
     * buffers owned by another manager or library.
     */
    fun stopAll() {
    ...
    private fun postCallbackEvent(
        ...
        sdkPreloadId: String,
        generation: Long,
        notify: (RewardedInterstitialAdLoadListener) -> Unit,
    ) {
        postToMain {
            val event = synchronized(lock) {
                val pool = currentPoolLocked(
    ...
    private fun stateLocked(pool: Pool): RewardedInterstitialAdLoadState {
        val availableCount = if (pool.isStarted && isSdkInitialized()) {
            gateway.getNumAdsAvailable(pool.sdkPreloadId)
        } else {
            0
        }
        return RewardedInterstitialAdLoadState(
    ...
    private fun stoppedState(pool: Pool): RewardedInterstitialAdLoadState {
        return RewardedInterstitialAdLoadState(
            isStarted = false,
            availableCount = 0,
            isLoading = false,
    ...
    private fun dispatchStateChanged(
        key: String,
        onlyListener: RewardedInterstitialAdLoadListener? = null,
    ) {
        postToMain {
            val event = synchronized(lock) {
    ...
    private data class Pool(
        ...
        var config: RewardedInterstitialAdLoadConfig,
        val listeners: MutableSet<RewardedInterstitialAdLoadListener> = linkedSetOf(),
    ...
    private data class CallbackEvent(
        val listeners: List<RewardedInterstitialAdLoadListener>,
        val state: RewardedInterstitialAdLoadState,
    )

    companion object {
        const val DEFAULT_AWAIT_TIMEOUT_MS: Long = 10_000L

        private const val SDK_PRELOAD_ID_PREFIX = "nextgen-rewarded-interstitial-"
        ...
        internal fun createForTesting(
            gateway: RewardedInterstitialAdPreloaderGateway,
            isSdkInitialized: () -> Boolean,
            elapsedRealtime: () -> Long,
            postToMain: (() -> Unit) -> Unit,
            registerInitListener: (listener: () -> Unit) -> Unit = {},
        ): RewardedInterstitialAdLoadManager = RewardedInterstitialAdLoadManager(
            gateway = gateway,
...
internal interface RewardedInterstitialAdPreloaderGateway {
    fun start(
        preloadId: String,
    ...
    fun pollAd(preloadId: String): RewardedInterstitialAd?
...
private object SdkRewardedInterstitialAdPreloaderGateway : RewardedInterstitialAdPreloaderGateway {
    override fun start(
        preloadId: String,
        configuration: PreloadConfiguration,
        callback: PreloadCallback,
    ): Boolean = RewardedInterstitialAdPreloader.start(preloadId, configuration, callback)
...
private fun RewardedInterstitialAd.responseIdOrNull(): String? =
